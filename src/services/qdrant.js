...
async function passageSparseVector(text) {
  ...
  for await (const batch of model.passageEmbed([text])) {
    return { indices: Array.from(batch[0].indices), values: Array.from(batch[0].values) };
  }
}
...
async function querySparseVector(text) {
  ...
  return { indices: Array.from(result.indices), values: Array.from(result.values) };
}
...
async function searchPoints(denseVector, sparseVector, limit = 5, scoreThreshold = 0.3) {
...
module.exports = { ensureCollection, upsertPoints, searchPoints, passageSparseVector, querySparseVector };
